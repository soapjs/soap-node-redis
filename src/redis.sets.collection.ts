 ...
 */
export class RedisSetCollection<T> implements Collection<T> {
  /**
  ...
  public async insert(records: T[]): Promise<T[]> {
    try {
      ...
      const command = ["SADD", collectionName, ...records.map(String)];
  ...
  public async findOne(member: string): Promise<boolean> {
    try {
      const command = ["SISMEMBER", this.collectionName, member];
  ...
  public async remove(params: RedisRemoveQueryParams): Promise<RemoveStats> {
    try {
      ...
      const command = ["SREM", collectionName, ...params.map(String)];
